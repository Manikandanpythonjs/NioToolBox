...
class TextToWord(customtkinter.CTk):
    def __init__(self):
        super().__init__()
        ...
        self.logotitle = customtkinter.CTkLabel(
            self, text="TEXT TO WORD", text_color="grey")
        self.logotitle.pack(padx=10, pady=10, anchor="n")

        self.openfile = customtkinter.CTkButton(
            self, text="Change Text to WORD ", command=self.open_file)
    ...
    def open_file(self):
        ...
        if self.file_path:
            ...
            self.openfile.configure(
                text="Convert to WORD", command=self.converttexttoword)
    ...
    def converttexttoword(self):
        ...
        if (self.file_path):
            ...
            self.openfile.configure(
                text="Change Text to Word", command=self.open_file)
