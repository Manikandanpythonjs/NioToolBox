...
class PpttoPdf(customtkinter.CTk):
    def __init__(self):
        super().__init__()
        ...
        self.logotitle = customtkinter.CTkLabel(
            self, text="PPTX TO PDF", text_color="grey")
        self.logotitle.pack(padx=10, pady=10, anchor="n")

        self.openfile = customtkinter.CTkButton(
            self, text="Change PPTX to PDF ", command=self.open_file)
    ...
    def open_file(self):
        ...
        if self.file_path:
            ...
            self.openfile.configure(
                text="Convert to PDF", command=self.convertppttopdf)
            self.selectedpath.set(self.file_path)
        self.attributes('-topmost', True)

    def convertppttopdf(self):
        ...
        if self.file_path:
            ...
            deck = powerpoint.Presentations.Open(self.file_path)
            deck.SaveAs(self.output_pdf_file, 32)
            deck.Close()

            powerpoint.Quit()
            self.wm_attributes("-topmost", True)
            self.openfile.configure(
                text="Change PPTX to PDF", command=self.open_file)
